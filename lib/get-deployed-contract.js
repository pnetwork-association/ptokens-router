...
const { getRouterContractAbi } = require('./get-contract-artifacts')
...
const getDeployedRouterContract = _deployedContractAddress =>
  console.info(`✔ Getting pToken contract @ '${_deployedContractAddress}'...`) ||
  getEnvConfiguration()
    .then(() => getEnvironmentVariable(ENDPOINT_ENV_VAR_KEY))
    .then(getProvider)
    .then(checkEndpoint)
    .then(_endpoint => Promise.all([ getEthersWallet(_endpoint), getRouterContractAbi() ]))
...
module.exports = { getDeployedRouterContract }
