...
const { getFeeContractState } = require('./get-fee-contract-state')
...
const getRouterState = _routerAddress =>
  getDeployedRouterContract(_routerAddress)
    .then(_routerContract =>
      Promise.all([
        getVaultInfoFromRouter(_routerContract),
        getFeeContractAddressFromRouter(_routerContract),
        _routerContract.signer,
      ])
    )
    .then(([ _vaultInfo, _feeContractAddress, _wallet ]) =>
      Promise.all([ _vaultInfo, getFeeContractState(_feeContractAddress, _wallet) ])
    )
    .then(([ _vaultInfo, _feeContractInfo ]) =>
      console.dir(assoc('feeContractInfo', _feeContractInfo, _vaultInfo), { depth: null })
    )
    .catch(console.warn)
