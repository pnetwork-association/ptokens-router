...
const { FEE_CONTRACT_ABI } = require('./abis/fee-contract-abi.json')
...
const getFeeContract = curry((_signer, _address) =>
  new ethers.Contract(_address, FEE_CONTRACT_ABI, _signer)
)
...
const getFeeContractAndAddToState = _state =>
  getFeeContract(_state[WALLET_STATE_KEY], _state[FEE_CONTRACT_ADDRESS_STATE_KEY])
    .then(_contract => assoc(FEE_CONTRACT_STATE_KEY, _contract, _state))
...
const getChainMultipliersAndAddToState = _state => {
  const feeContract = _state[FEE_CONTRACT_ADDRESS_STATE_KEY]

  Promise.all(CHAIN_IDS_HEX.map(_chainId => feeContract.MULTIPLIER(_chainId)))
    .then(_multipliers => {
      _multipliers.reduce((_acc, _, _i) => {
        const cid = CHAIN_IDS_HEX[_i]
        const key = `${getHumanReadableOriginChainIdFromHex(cid)} (${cid})`
        _acc[key] = _multipliers[_i]
        return _acc
      }, {})
    })
...
const getGlobalNodeOperatorsMinFeeAndAddToState = _state =>
  _state[FEE_CONTRACT_STATE_KEY]
    ...
    .then(_v => assoc(GLOBAL_MIN_FEE_STATE_KEY, _v, _state))
...
// TODO get fee balances for each sink address
