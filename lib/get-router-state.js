...
const DEFAULT_PEG_IN_FEE = 10 // TODO Get this from the contract and use that value instead!
const DEFAULT_PEG_OUT_FEE = 25 // TODO Get this from the contract and use that value instead!
...
const CUSTOM_FEE_STATE_KEY = 'customFees'
...
const FEE_CONTRACT_TOKEN_BALANCES_STATE_KEY = 'feeContractTokenBalances'
const CHAIN_IDS_HEX = CHAIN_ID_NAMES.map(_key => metadataChainIds[ _key ])
const NODE_OPERATORS_FEE_SINK_ADDRESS_STATE_KEY = 'nodeOperatorsFeeSinkAddress'
const SAFE_VAULT_CONTRACT_TOKEN_BALANCES_STATE_KEY = 'safeVaultContractTokenBalances'
const NODE_OPERATORS_FEE_SINK_ADDRESS_CONTRACT_KEY = 'NODE_OPERATORS_FEE_SINK_ADDRESS'
...
const getCustomFeesAndAddToState = _state => {
  const fxnName = 'getFeeBasisPoints'
  const feeContract = _state[FEE_CONTRACT_STATE_KEY]
  const tokenAddresses = _state[TOKEN_ADDRESSES_STATE_KEY]
  const tokenSymbols = _state[TOKEN_INFOS_STATE_KEY].map(_info => _info.symbol)

  if (tokenAddresses.length !== tokenSymbols.length)
    return Promise.reject(new Error('Number of token addresses does NOT match number of token symbols!'))

  return Promise.all([
    Promise.all(
      tokenAddresses.map(_address => callReadOnlyFxnInContract(fxnName, [ true, _address ], feeContract))
    ),
    Promise.all(
      tokenAddresses.map(_address => callReadOnlyFxnInContract(fxnName, [ false, _address ], feeContract))
    ),
  ])
    .then(([ pegInFeeArr, pegOutFeeArr ]) => {
      const obj = tokenSymbols.reduce((_obj, _symbol, _i) => {
        const pegInFee = parseInt(pegInFeeArr[_i])
        const pegOutFee = parseInt(pegOutFeeArr[_i])

        if (pegInFee === DEFAULT_PEG_IN_FEE && pegOutFee === DEFAULT_PEG_OUT_FEE) {
          // NOTE: If fees are not different to the default ones, we don't add them to this object
          return _obj
        } else {
          return assoc(tokenSymbols[_i], { pegInFee, pegOutFee }, _obj)
        }
      }, {})

      return assoc(CUSTOM_FEE_STATE_KEY, obj, _state)
    })
}
...
const getFeeContractStateAndAddToState = _state =>
  Promise.all(
    ...
    .then(assoc('feeSinkTokenBalances', _state[FEE_CONTRACT_TOKEN_BALANCES_STATE_KEY]))
    .then(assoc('customFees', _state[CUSTOM_FEE_STATE_KEY]))
...
const getNodeOperatorsFeeSinkAddressAndAddToState = _state =>
  callReadOnlyFxnInContract(
    NODE_OPERATORS_FEE_SINK_ADDRESS_CONTRACT_KEY,
    EMPTY_FXN_ARGS,
    ...
    .catch(_ => Promise.resolve(ZERO_ADDRESS))
    .then(_address => assoc(NODE_OPERATORS_FEE_SINK_ADDRESS_STATE_KEY, _address, _state))
...
const getFeeSinkAddressTokenBalancesAndAddToState = _state =>
  getAllTokenBalances(
    ...
    _state[NODE_OPERATORS_FEE_SINK_ADDRESS_STATE_KEY]
  )
    .then(_balances => assoc(FEE_CONTRACT_TOKEN_BALANCES_STATE_KEY, _balances, _state))
...
const prepareOutputAndPutInState = _state => {
  const output = {
    ...
    'feeContractInfo': _state[FEE_CONTRACT_STATE_STATE_KEY],
...
const getRouterState = _routerAddress =>
  addRouterAddressToState(INITIAL_STATE, _routerAddress)
    ...
    .then(getNodeOperatorsFeeSinkAddressAndAddToState)
    .then(getCustomFeesAndAddToState)
