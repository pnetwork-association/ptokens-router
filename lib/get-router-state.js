...
const getVaultInfoFromRouterAndAddToState = _state => {
  const vaultDictionary = _state[VAULT_DICTIONARY]
  const vaultNetworks = _state[VAULT_NETWORKS_STATE_KEY]
  const vaultAddresses = vaultNetworks.map(_key => vaultDictionary[ _key ])
  const vaultContracts = getVaultContracts(_state[ROUTER_CONTRACT_STATE_KEY].signer, vaultAddresses)
  return getVaultInfo(vaultNetworks, vaultContracts)
