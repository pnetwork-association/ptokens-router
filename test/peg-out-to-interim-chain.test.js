...
describe('Peg Out To Interim Chain Tests', () => {
  ...
  METADATA_VERSIONS.map(_metadataVersion =>
    it(`Should peg out to interim chain successfully using metadata version ${_metadataVersion}`, async () => {
      // NOTE: Set up the token contract that we'll use...
      const tokenContractChainId = INTERIM_CHAIN_ID
      // NOTE: This chain ID is what will cause this tx to be seen as a peg out!
